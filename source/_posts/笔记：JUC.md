...
##### 3.4.3.1. ReentrantLock 与 synchronized 的区别
...
##### 3.5.2.1. AtomicBoolean

----


##### 3.5.2.2. AtomicInteger
...
##### 3.5.2.3. AtomicIntegerArray

---


##### 3.5.2.4. AtomicIntegerFieldUpdater
...
##### 3.5.2.6. AtomicLongArray

---


##### 3.5.2.7. AtomicLongFieldUpdater
...
##### 4.1.1.1. AtomicReference

----


##### 4.1.1.2. AtomicReferenceArray

----



##### 4.1.1.3. AtomicReferenceFieldUpdater

----


##### 4.1.1.4. AtomicMarkableReference

----


##### 4.1.1.5. AtomicStampedReference
...
#### 1.5.3. 使用 FutureTask + Thread

##### 1.5.3.1. Runnable 的缺陷
...
1. 线程状态
	...
	2. RUNNABLE
		...
		2. 运行状态
			1. 线程被分配到 CPU 时间片后开始执行代码
		3. 阻塞状态
			1. 在 Java 层面的 BLOCKED、WAITING、TIMED_WAITING、阻塞状态 在操作系统层面都属于阻塞状态，CPU 不再调用这个线程
			2. Java 层面的阻塞状态，一般是我们执行的某些操作，涉及到操作系统层面的线程阻塞时，会进入阻塞状态，最常见的就是 I/O 操作
			3. 虽然其在操作系统层面进入阻塞状态，CPU 不再调用这个线程，但是从 JVM 的角度看，执行 IO 操作的线程通常仍然停留在 `RUNNABLE` 状态
			4. 常见的阻塞 I/O 操作有：
				1. 文件 I/O
				2. 网络 I/O
				3. 数据库 I/O
				4. 用户交互
				5. 需要注意的是，传统的这些操作确实属于阻塞 I/O，但随着技术的发展，Java 为这些操作提供了 BIO、NIO 和 AIO 等多种实现方式
	3. BLOCKED
		1. 使用 `synchronized(Object)` 获取对象锁时，如果竞争失败，线程会被加入该对象的 EntryList 阻塞队列中等待唤醒
	4. WAITING
		1. 线程正在等待被唤醒，它不会自动恢复，必须依赖外部条件
	5. TIMED_WAITING
		1. 线程正在等待被唤醒，但只等待一段有限时间，超时后，线程进入可运行状态，准备等待 CPU 时间片继续向下执行
	6. TERMINATED
		1. 当线程执行完代码后，会进入 `TERMINATED` 状态。
		2. 在 Java 层面上，线程是一个线程对象，当这个线程对象不再被引用时，会在下一次 GC 时被垃圾回收。
2. 线程状态转换
	1. NEW ---> 可运行状态
		1. 当通过 `new` 创建的线程对象调用 `start()` 方法后，Java 中的线程对象会与底层操作系统的原生线程建立关联，此时线程就进入了可运行状态
	2. 可运行状态 ---> 运行状态
		1. 线程被分配到 CPU 时间片后开始执行代码
	3. 运行状态 ---> 可运行状态
		1. 调用 `Thread.yield()`
		2. CPU 时间片到时
	4. 运行状态 ---> 阻塞状态
		1. 我们执行的某些操作，涉及到操作系统层面的线程阻塞时，会进入阻塞状态，最常见的就是 I/O 操作
	5. 阻塞状态 ---> 可运行状态
		1. I/O 数据就绪后，内核通知线程继续执行
	6. 运行状态 ---> BLOCKED
		1. 竞争对象锁失败
			1. 使用 `synchronized(Object)` 获取对象锁时，如果竞争失败，线程会被加入该对象的 EntryList 阻塞队列中等待唤醒
	7. BLOCKED ---> 可运行状态
		1. 拥有对象锁的线程执行完代码后，会通知处于 EntryList 中的线程，此时这些线程会被唤醒，尝试去争抢该对象的锁
	8. 运行状态 ---> WAITING
		1. 调用 `xxObject.wait()`
		2. 调用 `xxThread.join()`
		3. 调用 `LockSupport.park()`
	9. WAITING ---> 可运行状态
		1. 调用 `xxObject.notify()`、`xxObject.notifyAll()`
		2. 调用 `xxThread.interrupt()`
		3. 调用 `LockSupport.unpark(Thread)`
		4. 注意事项：
			1. 被 `xxObject.notify()`、`xxObject.notifyAll()`、`xxThread.interrupt()` 唤醒的 `wait` 线程，会尝试竞争对象锁
			2. 若成功获取锁，线程将进入可运行状态，准备等待 CPU 时间片继续向下进行（包括异常处理中的 `catch` 代码块）
			3. 若竞争失败，线程会进入 EntryList 阻塞队列，处于 `BLOCKED` 状态
	10. 运行状态 ---> TIMED_WAITING
		1. 调用 `xxObject.wait(long)`
		2. 调用 `xxThread.join(long)`
		3. 调用 `Thread.sleep(long)`
		4. 调用 `LockSupport.parkNanos(long)`、`LockSupport.parkUntil(long)`
	11. TIMED_WAITING ---> 可运行状态
		1. 等待时间到期，线程自动唤醒
		2. 调用 `xxObject.notify()`、`xxObject.notifyAll()`
		3. 调用 `xxThread.interrupt()`
		4. 调用 `LockSupport.unpark(Thread)`
		5. 注意事项：
			1. 被 `xxObject.notify()`、`xxObject.notifyAll()`、`xxThread.interrupt()` 唤醒的 `wait` 线程，会尝试竞争对象锁
			2. 若成功获取锁，线程将进入可运行状态，准备等待 CPU 时间片继续向下进行（包括异常处理中的 `catch` 代码块）
			3. 若竞争失败，线程会进入 EntryList 阻塞队列，处于 `BLOCKED` 状态
...
###### 1.7.1.1.1. sleep(long) 概述

| 方法名                | static | 说明                                       | 注意事项                                                                                                                                                                                                                                                                                                                                                          |
| ------------------ | ------ | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Thread.sleep(long) | static | 让**当前执行的线程**休眠 n 毫秒，休眠时会让出 cpu 的时间片给其它线程 | 1. 从运行状态 ---> TIMED_WAITING<br><br>2. 其他线程可以通过 `xxThread.interrupt()` 方法打断某个正在 sleep 的线程，此时 `Thered.sleep()` 方法会抛出 `InterruptedException`（如果你未捕获并处理该异常，线程可能直接终止）<br><br>3. 如果线程在睡眠结束或被中断（并正确处理了异常），将会从 TIMED_WAITING ---> 可运行状态，准备等待CPU时<br>间片继续向下执行<br><br>4. 建议使用 TimeUnit 的 sleep 代替 Thread 的 sleep，两者功能相同，但前者可读性更强，单位更清晰。<br><br>5. 以 毫秒 为单位，1000 毫秒是 1 秒 |
...
| 方法名            | static | 说明                                          | 注意事项                                                                                                                                                                                                    |
| -------------- | ------ | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Thread.yield() | static | 提示线程调度器让出**当前执行的线程**对 CPU 的使用，线程从运行状态进入就绪状态 | 1. 运行状态 ---> 可运行状态<br><br>2. 这个方法主要是为了测试和调试，并且仅是建议性提示，是否生效取决于具体的调度策略；<br><br>3. 与 `sleep()` 方法的区别在于：`yield()` 让出时间片后，线程会处于可运行状态，如果没有其他可运行的线程，当前线程仍有可能被继续调度执行；而 `sleep()` 会使线程进入`TIMED_WAITING`，不能随即就被调度 |
...
| 方法名                  | static | 说明  | 注意事项                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| -------------------- | ------ | --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| xxThread.interrupt() |        |     | 1. 主要用于打断处于等待状态（WAITING / TIMED_WAITING）状态的线程，被打断的线程将处于可运行状态<br><br>2. 如果被打断的线程是 `Thread.sleep(long)`、`xxThread.join()`、`xxObject.wait()` 而进睡眠状态的线程，将抛出 `InterruptedException`（如果你未捕获并处理该异常，线程可能直接终止；如果合理处理，将进入可运行状态）<br><br>3. 如果被打断的线程是 `LockSupport.park()`、`LockSupport.parkUntil(long)`、`LockSupport.parkUntil(long)` 而进入睡眠状态的线程，不会抛出异常，进入可运行状态（但）<br><br>4. 如果被打断的线程是运行状态的线程，那么不会中断线程的执行，但是会设置线程的中断标志位（isInterrupted() 为 true），如果线程代码中主动检查了中断状态（例如通过 `Thread.interrupted()`），就可以自行做出响应，比如提前退出或者其他逻辑 |
...
##### 1.7.2.4. join()、join(long)

| 方法名                 | static | 说明                               | 注意事项                                                                                                                                                                                                                                                                               |
| ------------------- | ------ | -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| xxThread.join()     |        | 让当前线程等待 xxThread 线程运行结束          | 1. 运行状态 ---> WAITING<br><br>2. 其他线程可以通过 `xxThread.interrupt()` 方法打断某个线程继续等待其他线程结束，此时 `xxThread.join()` 方法会抛出 `InterruptedException`（如果你未捕获并处理该异常，线程可能会直接终止；如果合理处理，将进入可运行状态）<br><br>3. 例如线程 b 正在等待线程 a 执行完毕，这时可以在 线程 c 中调用 `b.interrupt()` 来打断线程 b 的等待操作。<br><br>4. 需要考虑虚假唤醒情况（不必须） |
| xxThread.join(long) |        | 让当前线程等待 xxTread 线程运行结束，最多等待 n 毫秒 | 1. 运行状态 ---> TIMED_WAITING<br><br>2. 超时后，线程不再等待，进入可运行状态，准备继续向下执行<br><br>3. 以 毫秒 为单位，1000 毫秒是 1 秒                                                                                                                                                                                   |
...
| 方法名                 | static | 说明                                                                     | 注意事项                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------------------- | ------ | ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| xxObject.wait()     |        | 使当前持有该对象锁的线程进入该对象监视器（Monitor）中的 WaitSet 中等待，同时释放对象锁和 CPU 执行权           | 1. 运行状态 ---> WAITING（虽然处于可运行状态的线程也可能持有锁，但调用 `xxObject.wait()` 方法时，线程一定处于运行状态）<br><br>2. 与 `Thread.sleep()` 的区别是，`Thread.sleep()` 使线程进入 TIMED_WAITING 状态，但不会释放所持有的对象锁；而 `xxObject.wait()` 会使线程进入 WAITING 状态，并且会释放所持有的对象锁<br><br>3. `xxObject.wait()` 一定要放在 `synchronized` 加锁代码块中，否则会抛异常<br><br>4. 其他线程可以通过 `xxThread.interrupt()` 方法打断某个正在 wait 的线程，此时 `xxObject.wait()` 方法会抛出 `InterruptedException`（如果你未捕获并处理该异常，线程可能会直接终止；如果合理处理，将进入下面的流程）<br><br>5. 被 `xxObject.notify()`、`xxObject.notifyAll()`、`xxThread.interrupt()` 唤醒的 `wait` 线程，会尝试竞争对象锁<br><br>6. 若成功获取锁，线程将进入可运行状态，准备等待 CPU 时间片继续向下进行（包括异常处理中的 `catch` 代码块）<br><br>7. 若竞争失败，线程会进入 EntryList 阻塞队列，处于 `BLOCKED` 状态<br><br>8. 需要考虑虚假唤醒情况（必须考虑）<br><br>9. 并不是只有 `wait` 才需要考虑虚假唤醒，`park` 和 `join` 也可能发生虚假唤醒，但大多数情况下使用者不会主动检查，然而 `wait` 是官方文档明确要求**必须通过条件判断进行防护**的 |
| xxObject.wait(long) |        | 使当前持有该对象锁的线程进入该对象监视器（Monitor）中的 WaitSet 中等待，同时释放对象锁和 CPU 执行权，最多等待 n 毫秒 | 1. 运行状态 ---> TIMED_WAITING<br><br>2. 超时后，线程不再等待，进入可运行状态，准备继续向下执行<br><br>3. 单位是毫秒，1_000 毫秒是 1 秒                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
```
...
##### 1.7.3.2. notify()、notifyAll() 方法

| 方法名                | static | 说明                                                  | 注意事项                                                                                                                                                                                              |
| ------------------ | ------ | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| xxObject.notify()  |        | 当前持有该对象锁的线程去唤醒在<br>该对象监视器（Monitor）中 WaitSet 中随机一个线程 | 1. 被唤醒的线程由 WAITING / TIMED_WAITING ---> 可运行状态 / BOLCKED，但不要忘记，这些方法都是在 `synchronized` 代码块中的，也就是说还需要再获得 CPU 时间片并成功获取该对象锁后才能继续执行后续代码<br><br>2. `xxObject.notify()` 一定要放在 `synchronized` 代码块中，否则会抛异常。 |
...
#### 1.7.4. LockSupport 静态方法

##### 1.7.4.1. park()、parkNanos(long)、parkUntil(long)

| 方法名                         | static | 说明               | 注意事项                                                                                                                                                                                                                                                                    |
| --------------------------- | ------ | ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| LockSupport.park()          | static | 无限期阻塞当前线程        | 1. 运行状态 ---> WAITING<br><br>2. 与 `Thread.sleep(long)` 和 `xxThread.interrupt()` 不同的是，park 的线程可以被提前唤醒（即 `LockSupport.unpark(Thread)` 可以在 `LockSupport.park()` 之前就调用）<br><br>3. 其他线程可以通过 `xxThread.interrupt()` 方法打断某个正在 park 的线程，不会抛出异常，进入可执行状态<br><br>4. 需要考虑虚假唤醒情况（不必须） |
| LockSupport.parkNanos(long) | static | 阻塞当前线程，最多阻塞 n 纳秒 | 1. 运行状态 ---> TIMED_WAITING<br><br>2. 超时后，线程不再等待，进入可运行状态，准备继续向下执行<br><br>3. 单位是纳秒，1_000_000_000 纳秒 = 1 秒                                                                                                                                                                 |
| LockSupport.parkUntil(long) | static | 阻塞当前线程到指定时间点     | 1. 运行状态 ---> TIMED_WAITING<br><br>2. 超时后，线程不再等待，进入可运行状态，准备继续向下执行<br><br>3. 参数是 Unix 毫秒级时间戳，如 `System.currentTimeMillis() + 3000` 是暂停三秒                                                                                                                                  |
```
public class Main {  
    ...
    public static void main(String[] args) throws InterruptedException {  
        ...
        Thread myThread = new Thread(() -> {  
            ...
            System.out.println("myThread 进行 park");  
            LockSupport.park();  
            System.out.println("myThread 从 park 恢复，继续后续代码");  
			  
        });  
			  
        myThread.start();  
			  
        // 主线程 sleep 50 豪秒后恢复 myThread        
        Thread.sleep(50);  
        System.out.println("主线程恢复 myThread"); 
...
> [!NOTE] 注意事项
> 1. 提前恢复 `LockSupport.park()` 的线程示例代码：
```
public class Main {  
...
| 方法名                        | static | 说明        | 注意事项                  |
| -------------------------- | ------ | --------- | --------------------- |
| LockSupport.unpark(Thread) | static | 恢复某个线程的运行 | 1. WAITING ---> 可运行状态 |
```
public class Main {  
